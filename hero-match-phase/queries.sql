...
select mymatch.id as matchid, bs_info.index,gpm, hero_name, match_time, (bs_info.index < 5 && winner=2) or (bs_info.index >= 5 && winner=3) as winner, bs_info.hero_num into outfile '/tmp/hero_performance.csv' FIELDS TERMINATED BY ','LINES TERMINATED BY '\n' from bs_info join herodisp on bs_info.hero_num = herodisp.hero_num join mymatch on mymatch.id=bs_info.id join basic_stats2 on basic_stats2.id=bs_info.id and basic_stats2.index=bs_info.index;